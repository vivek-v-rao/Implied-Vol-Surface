#!/usr/bin/env python3
"""
Fit CEV smiles per expiration from European option prices in a CSV file.
...
from cev import cev_price, fit_cev
...
def main(argv: list[str]) -> None:
    """CLI entrypoint."""
    if len(argv) < 2:
        print("usage: python xcev.py options.csv [--expiry YYYY-MM-DD|YYYYMMDD:YYYYMMDD] [--as-of YYYY-MM-DD]")
        print("       [--fwd-range L:U] [--both-sides] [--beta B] [--fixed-beta] [--beta-grid spec] [--outfile out.csv]")
    ...
    expiry_range = None
    beta = 0.7
    fit_beta = "--fixed-beta" not in argv
    if "--fit-beta" in argv:
        fit_beta = True
    beta_grid_spec = None
    ...
    if "--beta-grid" in argv:
        beta_grid_spec = argv[argv.index("--beta-grid") + 1]
    beta_grid = None
    if beta_grid_spec:
        parts = beta_grid_spec.split(":")
        if len(parts) == 3:
            start, end, step = (float(p) for p in parts)
            if step <= 0:
                raise ValueError("beta-grid step must be > 0")
            n = int(round((end - start) / step)) + 1
            beta_grid = [start + i * step for i in range(n)]
        elif len(parts) == 1:
            beta_grid = [float(parts[0])]
        else:
            raise ValueError("beta-grid must be start:end:step or a single value")
    ...
    for exp in expiries:
        ...
        sigma0 = float(np.median(work["iv"].values.astype(float)))

        if beta_grid is not None:
            grid_rows = []
            for b in beta_grid:
                params_b = fit_cev(spot, rate, t, strikes, prices, opt_types,
                                   beta=b, fit_beta=False, q=q, sigma0=sigma0)
                grid_rows.append(
                    {
                        "expiry": exp,
                        "beta": b,
                        "sigma": params_b["sigma"],
                        "rmse": params_b["rmse"],
                    }
                )
            if grid_rows:
        ...
        params = fit_cev(spot, rate, t, strikes, prices, opt_types, beta=beta, fit_beta=fit_beta, q=q, sigma0=sigma0)
        results.append(
            {
                "expiry": exp,
                "forward": forward,
        ...
        for k in strikes_sorted:
            price = cev_price("c", spot, k, rate, t, params["sigma"], params["beta"], q)
            try:
                ivf = implied_vol("c", price, spot, k, rate, t, q, model="european")
            except Exception:
                ivf = float("nan")
            iv_fit.append(ivf)
        iv_fit = np.asarray(iv_fit, dtype=float)
        for strike, ivf in zip(strikes_sorted, iv_fit):
            if not np.isfinite(ivf):
                continue
            fit_rows.append(
                {
                    "expiry": exp,
    ...
    if plot and plot_data:
        ...
        ax.set_title(f"CEV implied vol vs strike by expiry ({title_sym}, {title_date})")
    ...
    if plot_density and density_data:
        ...
        for exp, strikes, forward, dfactor, t, params in density_data:
            ...
            call_prices = np.array(
                [cev_price("c", spot, kk, rate, t, params["sigma"], params["beta"], q) for kk in grid]
            )
            dK = grid[1] - grid[0]
        ...
        title_sym = symbol if symbol is not None else "data"
        if plot_density_log_s:
            ax.set_title(f"Implied density vs log stock (CEV, {title_sym}, {title_date})")
            ax.set_xlabel("log(stock)")
        else:
            ax.set_title(f"Implied density vs stock (CEV, {title_sym}, {title_date})")
            ax.set_xlabel("stock")
