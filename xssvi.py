#!/usr/bin/env python3
"""
Fit SSVI surface across expirations from European option prices in a CSV file.
...
Usage:
  python xssvi.py options.csv [--expiry YYYY-MM-DD|YYYYMMDD:YYYYMMDD] [--as-of YYYY-MM-DD]
      [--fwd-range L:U] [--both-sides] [--essvi] [--plot [file.png]] [--plot-density [file.png]]
      [--plot-density-log-s] [--outfile out.csv]
"""
...
def main(argv: list[str]) -> None:
    """CLI entrypoint."""
    if len(argv) < 2:
        print("usage: python xssvi.py options.csv [--expiry YYYY-MM-DD|YYYYMMDD:YYYYMMDD] [--as-of YYYY-MM-DD]")
        print("       [--fwd-range L:U] [--both-sides] [--essvi] [--plot [file.png]] [--plot-density [file.png]]")
        print("       [--plot-density-log-s] [--outfile out.csv]")
        sys.exit(1)
    ...
    for exp in expiries:
        ...
        rows.append({"expiry": exp, "forward": forward, "df": dfactor, "t": t})
        tenor_stats.append(
            {
                "expiry": exp,
                "forward": forward,
                ...
                "t": t,
                "num_points": len(work),
                "mean_iv_pct": float(np.mean(work["iv"]) * 100.0),
            }
        )

    if not all_k:
        print("no data to fit SSVI", file=sys.stderr)
        sys.exit(1)
    ...
    for row in rows:
        ...
        forward = row["forward"]
        t = row["t"]
        strikes = np.sort(df.loc[df["expiration"] == exp, "strike"].unique().astype(float))
        if strikes.size == 0:
            continue
        k = np.log(strikes / forward)
        if essvi:
            w_fit = essvi_total_variance(k, np.full_like(k, t), params["a"], params["b"], params["c"],
        ...
        for strike, iv in zip(strikes, iv_fit):
            fit_rows.append(
                {
                    "expiry": exp,
                    "strike": float(strike),
        ...
        plot_data.append((exp, strikes, iv_fit * 100.0))
        density_data.append((exp, strikes, forward, dfactor, t, params))
    ...
    if outfile and fit_rows:
        ...
        print(f"Wrote fitted vols to {outfile}")
    ...
    if plot_density and density_data:
        ...
        if plot_density_file:
            ...
            print(f"Wrote density plot to {plot_density_file}")
        else:
            plt.show()
